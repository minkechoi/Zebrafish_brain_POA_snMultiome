# =============================================================================
# 08_d_DEG_modules_net_06_2025v3.R
# -----------------------------------------------------------------------------
# Purpose : Integrate WGCNA module genes with differentially expressed genes
#           (DEGs) for focal stress clusters, build a combined gene x condition
#           expression matrix, and export a gene-gene co-expression network
#           (edges + nodes) for Cytoscape, plus summary heatmaps.
# Inputs  : ./data/rda/DEGs_step7.rda (postLD_preLD, bPAC_cont DEG lists)
#           per-library WGCNA results: data/<lib>/rda/danio_wgcna_all.rda,
#           data/<lib>/rda/danio_counts.rda, ./outputs/<lib>/ss_modules_table.xlsx
# Outputs : ./outputs/WGCNA_topQ_merged_mx.csv
#           ./outputs/net_/node_/edges_ wgcna_3545.*  (Cytoscape network files)
#           ./figures/cytoscape/wgcna_heatmap.pdf; ./figures/WGCNA_45_sst1.1_heatmap.pdf
# Focus   : clusters 35.0_avp.crhb, 35.1_avp, 45_sst1.1 (stress/CRH/SST neurons).
# =============================================================================


#module merge
#####
#load DEGs (differential expression results computed in step 07)
...
# --- Collect top module ("hub-like") genes per library ---
...
# For each library, keep the top-50 genes per WGCNA module by module membership (kME > 0.7)
top_genes_md=list()
for (i in libr) {
  load(file = paste0("data/",i,"/rda/danio_wgcna_all.rda"))

  gene_lists_by_column <- lapply(colnames(datKME), function(col) {
    genes <- rownames(datKME)[datKME[, col] > 0.7]      # strong module members
    val = datKME[genes,col]
    names(val)=genes
    val=sort(val, decreasing = TRUE)
    val <- head(names(val), 50)                          # top 50 by membership
    return(val)
  })
...
# --- Combine per-library module summary tables ---
merged_modules_sum=list()
for (i in libr) {
  module_sum=read_excel(paste0("./outputs/",i,"/ss_modules_table.xlsx"))
  module_sum$source=i                                    # remember which library each module came from
...
# Split and unnest the `celltypes` column (one module can map to several cell types)
library(tidyr)
library(dplyr)
...
# --- For each cell type, gather the top module genes across its modules ---
...
for (celltype in names(celltype_tables)) {
  ...
  if (nrow(df)>0) {
    for (i in seq_len(nrow(df))) {
      mod <- df$newname[i]                 # module name
      src <- df$source[i]                  # source library
...
#for 45_sst1.1 : make sure the marker sst1.1 is included
...
# Load the normalised pseudobulk count matrix per library
#norm_count_matrix
for (group in libr) {
  #{r load_data}
...
# --- Assemble the DEG + module gene set for the three focal clusters ---
list_test_cluster=c("35.0_avp.crhb", "35.1_avp", "45_sst1.1")

DEG_and_module=c()
for (cluster in list_test_cluster) {
  ncluster=gsub("_","\\.",cluster)
  # DEGs for this cluster from both the light/dark (postLD_preLD) and genotype (bPAC_cont) contrasts
  ...
  DEGs=na.omit(DEGs)
DEG_and_module=unique(c(DEGs,extracted_genes[[cluster]]))   # DEGs + module genes for this cluster
}

# --- Build gene x (cluster x library) expression matrix ---
mt0=as.data.frame(DEG_and_module)
rownames(mt0)=mt0[,1]
colnames(mt0)="gene"

# Pull the focal-cluster columns from each library's normalised counts
...
merged_df$gene <- NULL
merged_df=merged_df[rowSums(merged_df!=0) > 0, ]       # drop all-zero genes

write.csv(merged_df,paste0("./outputs/WGCNA_topQ_merged_mx.csv"),quote = F)

dir.create("./figures/cytoscape")

# --- Overview heatmap of the combined matrix (row z-scored) ---
...
# Use a taller canvas when there are many genes
if (nrow(mt)>30) {
...
#### Build the co-expression network for Cytoscape
...
# pivot the matrix to make it tidy (keep strong, non-redundant positive edges)
cor_long <- cor_df %>%
...
# write out network file (SIF: source interaction target)
...
# get node info (DEG): per-gene summary stats + DEG flag for Cytoscape node table
node_info <- merged_df
...
node_info_tb$sigDEG[which(node_info_tb$Gene %in% DEGs)]= "yes"   # mark significant DEGs
# write out nodes file
# change this filename if necessary
nodes_file <- paste0("wgcna_3545",'.txt')
write_tsv(node_info_tb, paste0("./outputs/node_",nodes_file))

# make edges df (human-readable "GeneA (geneExprCor) GeneB" format)
...
#### Per-cluster figure heatmap (Fig 3)
...
mt= as.matrix(t(scale(t(node_info_tb_45))))       # row z-score across the 4 conditions
