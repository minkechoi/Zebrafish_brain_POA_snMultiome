...
wgcna_graph=function(group){
  ...
  #{r load_data}
  TOM_2 <- readRDS(paste0("data/",group,"/rda/wgcna_TOM_matrix.rds"))
  #
  #{r}
  load(paste0("data/",group,"/rda/danio_counts.rda"))
  load(paste0("data/",group,"/rda/danio_wgcna_all.rda"))
  load(paste0("data/",group,"/rda/tf_analysis.rda"))
  load(paste0("data/",group,"/rda/motif_analysis.rda"))
  #
  ...
  class_color_df <- data.frame(
    TFclass = unique(danio_tfs$class),
    color = c("yellow","blue" )
  )
  colnames(class_color_df) <- c("TFclass","color")
  
  #merge everything together
  danio_tfs_graph_annotation <- danio_tfs
  colnames(danio_tfs_graph_annotation)[2] <- "TFclass"
  
  danio_tfs_graph_annotation <- merge(
    danio_tfs_graph_annotation,
    class_color_df,
    by.x = 2,
    by.y = 1,
    all.x = T
  ...
  # Functional categories (COG) from STRING
  library(org.Dr.eg.db)
  'library(STRINGdb)
  string_db <- STRINGdb$new( version="12.0", species=7955, 
                             score_threshold=200, network_type="full", 
                             #link_data="full", 
                             input_directory=paste0("./data/string_db"))
  string_db'
  string_prot=read.table(paste0("./data/string_db/7955.protein.info.v12.0.txt"),sep="\t")
  gene_ids <- string_prot[,1][
    na.omit(match(string_prot[,2],danio_id_module$id))
    ]
  
  danio_cogs_all = read.table(paste0("./data/string_db/7955.protein.orthology.v12.0.txt"))
  colnames(danio_cogs_all)=c("id","level","cog")
  gene_ids_cog = danio_cogs_all[which(danio_cogs_all$id %in% gene_ids),
                               c("id","cog")]
  gene_ids_cog$id= translate_ids(gene_ids_cog$id,string_prot[,c(1,2)])
  
  danio_functional_categories_graph_annotation <- gene_ids_cog # danio_cogs, loaded from the source data
  colnames(danio_functional_categories_graph_annotation) = c("id","funcat")
  danio_functional_categories_graph_annotation=danio_functional_categories_graph_annotation[c(grep("KOG",danio_functional_categories_graph_annotation$funcat)),]
  danio_functional_categories_graph_annotation=unique(danio_functional_categories_graph_annotation)
  #colnames(danio_cogs) <- c("id","cog")
  

  # Retrieve GO terms
  library(AnnotationDbi)
  gene_ids <- mapIds(org.Dr.eg.db, 
                     keys = danio_id_module$id, 
                     column = "ENTREZID", 
                     keytype = "SYMBOL", 
                     multiVals = "first")
  danio_go_all <- AnnotationDbi::select(org.Dr.eg.db, 
                                       keys = gene_ids , 
                                       columns = c("GO", "ONTOLOGY"), 
                                       keytype = "ENTREZID")
  gene_ids_go= mapIds(org.Dr.eg.db, 
                      keys = danio_go_all$ENTREZID, 
                      column = "SYMBOL", 
                      keytype = "ENTREZID", 
                      multiVals = "first")
  
  danio_go_all$gene=gene_ids_go
  danio_go=danio_go_all[,c(5,2)]
  danio_functional_go_graph_annotation <- danio_go #loaded from the source data
  colnames(danio_functional_go_graph_annotation) = c("id","funcat")
  
  # Module
  danio_modules_graph_annotation <- danio_id_module # loaded from the previous markdown
  danio_modules_graph_annotation$module <- as.character(danio_modules_graph_annotation$module)
  danio_modules_graph_annotation$module_color <- 
    translate_ids(
      x = danio_modules_graph_annotation$module,
      dict = danio_modules_table[,c(4,7)]
    )
  danio_modules_graph_annotation$module_border <- darken(danio_modules_graph_annotation$module_color,0.5)
  danio_modules_graph_annotation$cell_modulecolor <- translate_ids(x = danio_modules_graph_annotation$module, dict = danio_modules_table[,c(4,8)])
  danio_modules_graph_annotation$genecolor <- translate_ids(x = danio_modules_graph_annotation$id, dict = danio_genecolor[,c(1,2)])
  
  # Gene color
  #danio_genecolor_graph_annotation <- danio_genecolor
  
  # Merge
  danio_attributes_list <- list(
    danio_tfs_graph_annotation,
    danio_modules_graph_annotation,
    #danio_genecolor_graph_annotation,
    danio_functional_categories_graph_annotation,
    danio_functional_go_graph_annotation
  )
  #
  ...
  #{r generate_graph0}
  danio_graph0 <- graph_from_adjacency_matrix(
  ...
  #{r parse_danio_graph}
  danio_parsenetwork <- ParseNetwork(danio_graph0, danio_attributes_list)
  #
  ...
  #{r}
  pdf(paste0("figures/",group,"/WGCNA/histogram_wgcna.pdf"), height = 4, width = 8)
  par(mfrow = c(1,2))
  hist(TOM_2[1:100000000], xlim = c(0,1),ylim=c(1,10000), col = "black", breaks = 500, main = "WGCNA gene-gene\ninteractions (floored 10K)")
  hist(TOM_2[TOM_2>0.1][1:100000000], xlim = c(0,1), col = "black", breaks = 500, main = "WGCNA gene-gene\ninteractions (>.1)")
  abline(v=c(0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5),col = divergingx_hcl(9,"Spectral", rev = TRUE), lwd = 2)
  ...
  for (thresh in thresholds) {
    ...
    graph_iter <- subgraph.edges(
      danio_graph0,
      eids = which(E(danio_graph0)$weight >= thresh),
  ...
  saveRDS(list_metrics,paste0("./data/",group,"/rda/list_graph_metrics.rds"))
  #
  #{r, echo = FALSE}
  #list_metrics <- readRDS(paste0("./data/",group,"/rda/list_graph_metrics.rds"))
  #
  ...
  boxplot(
    ...
    names = gsub("danio_wg_graph_","",names(list_metrics)),
    notch=F,
    cex.axis=0.9,
    outline=F,
  ...
  #{r}
  pdf(paste0("./figures/",group,"/WGCNA/graph_behaviours.pdf"), height = 8, width = 8)
  par(mfrow = c(2,2))
  plot(
    seq(0.1,0.5,by=0.05),
    sapply(list_metrics,function(x){length(unique(x$member_weak))}),
  ...
  #{r}
  # 0.4 VALUES
  danio_graph_04 <- subgraph.edges(
    danio_parsenetwork[[1]],
    eids = which(E(danio_parsenetwork[[1]])$weight >= 0.4),
    delete.vertices = TRUE
  )
  danio_graph_04 = delete.vertices(danio_graph_04, which(igraph::degree(danio_graph_04)==0)) # watch out for circlize package here; also has a 'degree' function
  
  # 0.35 VALUES
  danio_graph_035 <- subgraph.edges(
    danio_parsenetwork[[1]],
    eids = which(E(danio_parsenetwork[[1]])$weight >= 0.35),
    delete.vertices = TRUE
  )
  danio_graph_035 = delete.vertices(danio_graph_035, which(igraph::degree(danio_graph_035)==0)) # watch out for circlize package here; also has a 'degree' function
  
  # 0.25 VALUES
  danio_graph_025 <- subgraph.edges(
    danio_parsenetwork[[1]],
    eids = which(E(danio_parsenetwork[[1]])$weight >= 0.25),
    delete.vertices = TRUE
  )
  danio_graph_025 = delete.vertices(danio_graph_025, which(igraph::degree(danio_graph_025)==0)) # watch out for circlize package here; also has a 'degree' function
  
  # 0.1 VALUES
  danio_graph_01 <- subgraph.edges(
    danio_parsenetwork[[1]],
    eids = which(E(danio_parsenetwork[[1]])$weight >= 0.1),
    delete.vertices = TRUE
  )
  danio_graph_01 = delete.vertices(danio_graph_01, which(igraph::degree(danio_graph_01)==0)) # watch out for circlize package here; also has a 'degree' function
  #
  ...
  barplot(
    main = "graph size",
    c(
      vcount(danio_graph_01),
      vcount(danio_graph_025),
      vcount(danio_graph_035)
    ),
    names.arg = c(">= .1",">= .25", ">=.35"),
    xlab = "threshold",
    ylab = "no. genes",
  ...
  #
  #{r}
  pdf(paste0("./figures/",group,"/WGCNA/graph_sizes.pdf"), height = 5, width =3 )
  barplot(
    main = "graph size",
    c(
      vcount(danio_graph_01),
  ...
  vc = sqrt(vcount(danio_graph_035))
  fr_temp = 0.1*vc
  ad = E(danio_graph_035)$weight/median(E(danio_graph_035)$weight)
  # ad[ad == min(ad)] = 1e-13
  ...
  danio_layout_fr <- 
    layout_with_fr(
      danio_graph_035,
      niter = 150,
      weights = E(danio_graph_035)$weight/vc,
      start.temp = fr_temp
    )
  #
  ...
  plot(
    ...
    vertex.size = 1.5,
    vertex.label = NA,
    vertex.color = V(danio_graph_035)$genecolor,
    vertex.frame.color = darken(V(danio_graph_035)$genecolor,0.5),
    edge.width = ad_b,
    edge.color = rgb(0,0,0,0.1),
    layout = danio_layout_fr #layout_components(danio_graph_035, layout=layout_with_fr, start.temp = 0.1)
  )
  #
  ...
  plot(
    ...
    vertex.size = 1.5,
    vertex.label = NA,
    vertex.color = V(danio_graph_035)$module_color,
    vertex.frame.color = V(danio_graph_035)$module_border,
    edge.width = ad_b,
    edge.color = rgb(0,0,0,0.1),
    layout = danio_layout_fr #layout_components(danio_graph_035, layout=layout_with_fr, start.temp = 0.1)
  )
  ...
  #{r}
  png(
    paste0("./figures/",group,"/WGCNA/danio_wgcna_FR.png"),
  ...
  plot(
    ...
    vertex.size = 1.5,
    vertex.label = NA,
    vertex.color = V(danio_graph_025)$module_color,
    vertex.frame.color = V(danio_graph_025)$module_border,
    edge.color = rgb(0,0,0,0.1),
    layout = danio_layout_kk
  )
  #
  ...
  plot(
    ...
    edge.width = ad_b,
    edge.color = rgb(0,0,0,0.1),
    layout = layout_components(danio_graph_035, layout=layout_with_fr,start.temp=0.1)
  )
  #
  
  #{r}
  png(paste0("./figures/",group,"/WGCNA/danio_wgcna_KK.png"),
      height = 1024, width = 1024, pointsize = 24
  )
  plot(
    main = "Kamada-Kawai",
  ...
  png(paste0("./figures/",group,"/WGCNA/danio_wgcna_CC.png"),
      height = 1024, width = 1024, pointsize = 24
  )
  plot(
    main = "FR (by Components)",
  ...
  #{r}
  danio_id_component <- data.frame(
    id = names(components(danio_graph_035,mode=c("strong"))$membership),
    member = components(danio_graph_035,mode=c("strong"))$membership
  ) %>% remove_rownames
  barplot(rev(sort(table(danio_id_component$member))), col = "#4c4c4c", border=NA)
  #
  
  #{r, fig.width = 10, fig.height = 4}
  #stacked barplot showing how many genes from every module are in each component
  danio_id_module_cc <- merge(danio_id_component,danio_id_module, by.x = 1, by.y = 1, all.x = TRUE)
  
  rand_index <- mclust::adjustedRandIndex(danio_id_module_cc$module,danio_id_module_cc$member)
  
  cc_table <- table(danio_id_module_cc$module,danio_id_module_cc$member)
  cc_table <- apply(cc_table,2,function(x){x/sum(x)})
  
  barplot(
    cc_table,
    col = translate_ids(x=rownames(cc_table), dict = danio_modules_table[,c(4,7)]),
    border = darken(translate_ids(x=rownames(cc_table), dict = danio_modules_table[,c(4,7)]),0.5),
  ...
  #{r}
  pdf(paste0("./figures/",group,"/WGCNA/barplot_CCs.pdf"),height = 3, width = 7)
  barplot(
    cc_table,
    col = translate_ids(x=rownames(cc_table), dict = danio_modules_table[,c(4,7)]),
    border = darken(translate_ids(x=rownames(cc_table), dict = danio_modules_table[,c(4,7)]),0.5),
  ...
  # TF CONNECTIVITY
  tfs_kme_intramodular <- danio_id_module[danio_id_module$id %in% danio_tfs$id,]
  ...
  # TF CENTRALITY
  danio_graph0_parse <- ParseNetwork(danio_graph0,list_attr = danio_attributes_list)
  
  danio_id_module_ <- danio_id_module
  colnames(danio_id_module_) <- c("id","member")
  
  danio_grns_list <- divide_into_components( 
    x = danio_graph0_parse[[1]],
    CCs = danio_id_module_
  )
  ...
  tfscentr_by_module <- centrality_by_network(danio_grns_list, normalized = TRUE)
  ...
  danio_tfs_centrality_df <- danio_tfs_centrality_df[,c(2,3,1,9,10,11)]
  
  colnames(danio_tfs_centrality_df) <- c("id","centrality","module","color","cellcolor","top_central")
  
  danio_tfs_centrality_df = danio_tfs_centrality_df[danio_tfs_centrality_df$module %in% names( table(danio_tfs_centrality_df$module)[ table(danio_tfs_centrality_df$module)>2]),]
  
  
  # CORRELATION OF BOTH
  kme_centr_all <- merge(
    tfs_kme_intramodular,
    danio_tfs_centrality_df[,-3],
    by.x = 1,
    by.y = 1,
    all = TRUE
  )
  ...
  # all together plot
  pdf(paste0("./figures/",group,"/WGCNA/scatter_cor_kme_all.pdf"), wi = 5, he = 5)
  plot(
    kme_centr$rel_kme,
    kme_centr$rel_centr,
  ...
  ## module-wise plot
  pdf(paste0("./figures/",group,"/WGCNA/scatter_cor_kme_modulewise.pdf"), he = 15, wi = 15)
  par(mfrow = c(6,6))
  ...
  d$module = rownames(d)
  d$col = translate_ids(d$module, danio_modules_table[,c(4,7)])
  ...
  pdf(paste0("./figures/",group,"/WGCNA/cor_kme_centr_jitter.pdf"), he = 5, wi = 5)
  set.seed(1234)
  ...
  set.seed(1234)
  par(mfrow = c(8,4))
  for (i in 1:length(danio_grns_list)) {
    set.seed(1)
    l <- layout_with_fr(danio_grns_list[[i]])
    pdf(paste0("./figures/",group,"/WGCNA/plot_separate_components_",i,".pdf"), he = 5, wi = 5)
    plot(
      main = i,
      danio_grns_list[[i]],
      vertex.size = 2,
      vertex.color = "gray",#V(danio_grns_list[[i]])$genecolor,
      vertex.frame.color = "gray50",#darken(V(danio_grns_list[[i]])$genecolor,0.5),
      vertex.label = NA,
      layout = l,
      edge.color = rgb(0,0,0,0.1)
    )
  ...
  #{r cross_connections}
  g_xconn <- danio_graph_01
  
  danio_cross_connections <- as.data.frame(
    t(
      data.frame(
        lapply(
          V(g_xconn)$name,
          ...
          network = g_xconn,
          id_module = danio_id_module[danio_id_module$id %in% V(g_xconn)$name, ] # * see below
        )
      )
    )
  )
  ...
  rownames(danio_cross_connections) <-  V(g_xconn)$name
  colnames(danio_cross_connections) <- levels(danio_modules_table$newname)
  
  danio_cross_connections <- merge(
    danio_cross_connections,
    danio_id_module,
    by.x = 0,
    by.y = 1
  )
  colnames(danio_cross_connections)[1] <- "id"
  
  str(danio_cross_connections)
  #
  ...
  size_cloud <- data.frame(
    table(danio_cross_connections$module)
  ...
  danio_cross_connections_norm <- as.data.frame(t(danio_cross_connections_norm))
  rownames(danio_cross_connections_norm) <- test$id
  colnames(danio_cross_connections_norm) <- colnames(test)[module_cols] # levels(danio_modules_table$newname)
  danio_cross_connections_norm$module <- test$module
  #
  ...
  module_cols <- which(!(colnames(danio_cross_connections_norm) %in% c("module","id","size")))
  filt_more_one_module_rows <- apply(
    danio_cross_connections_norm[,module_cols],
    1,
    function(x){
  ...
  filt_more_one_module_cols <- colSums(danio_cross_connections_norm[,module_cols])>0
  
  danio_cross_connections_norm <-
    danio_cross_connections_norm[filt_more_one_module_rows, filt_more_one_module_cols]
  #
  ...
  danio_cross_connections_norm_module <- stats::aggregate(
    danio_cross_connections_norm[module_cols],
    by = list(Module = danio_cross_connections_norm$module),
    FUN = sum, drop=F
  )
  
  rownames(danio_cross_connections_norm_module) <- danio_cross_connections_norm_module$Module
  danio_cross_connections_norm_module$Module <-  NULL
  #
  ...
  #qs <- quantile(as.numeric(as.matrix(danio_cross_connections_norm_module))[as.numeric(as.matrix(danio_cross_connections_norm_module))>0])
  qs <- c(0.00000000001,1,5,10,100)
  Heatmap(
    name = "no. connections\nscore",
    as.matrix(danio_cross_connections_norm_module[,module_cols]),
    col = colorRamp2(
      breaks = c(0,qs),
  ...
  danio_cross_connections_graph <- graph_from_adjacency_matrix(
    adjmatrix = as.matrix(danio_cross_connections_norm_module),
    add.colnames = "name",
    mode = "upper",
    weighted = TRUE,
    diag = FALSE,
  ...
  V(danio_cross_connections_graph)$color <- 
    translate_ids(x = V(danio_cross_connections_graph)$name, dict = danio_modules_table[,c(4,7)])
  V(danio_cross_connections_graph)$border <- 
    darken(V(danio_cross_connections_graph)$color,0.5)
  
  E(danio_cross_connections_graph)$width <- 
    as.numeric(as.character(
      cut(
        E(danio_cross_connections_graph)$weight,
        breaks = qs,
        labels = c(0.5,1,2,4),
        right=FALSE
      )
  ...
  danio_cross_connections_graph <- subgraph.edges(
    danio_cross_connections_graph,
    eids = which(E(danio_cross_connections_graph)$width > 0.5),
    delete.vertices = FALSE
  )
  danio_cross_connections_graph <- delete.vertices(danio_cross_connections_graph, which(igraph::degree(danio_cross_connections_graph)==0))
  #
  ...
  plot(
    ...
    layout = layout.graphopt(danio_cross_connections_graph),
    vertex.frame.color = V(danio_cross_connections_graph)$border,
  ...
  # tf_module_cor <- cor(t(danio_tfs_cw),MEs)
  ...
  tiff(paste0("./figures/",group,"/WGCNA/tf_connectivity_heatmap.tiff"),
       width = 15,height = 15,units = "cm", res = 300,compression = "lzw",bg = NA)
  ...
  V(g_m_tf_ppc)$color <- translate_ids(V(g_m_tf_ppc)$name, dict = danio_modules_table[,c(4,7)])
  V(g_m_tf_ppc)$border <- darken(translate_ids(V(g_m_tf_ppc)$name, dict = danio_modules_table[,c(4,7)]),0.5)
  
  tiff(paste0("./figures/",group,"/WGCNA/tf_connectivity_heatmap_divergingx_hcl.tiff"),
       width = 15,height = 15,units = "cm", res = 300,compression = "lzw",bg = NA)
  ...
  V(g_m_mot_ppc)$color <- translate_ids(V(g_m_mot_ppc)$name, dict = danio_modules_table[,c(4,7)])
  V(g_m_mot_ppc)$border <- darken(translate_ids(V(g_m_mot_ppc)$name, dict = danio_modules_table[,c(4,7)]),0.5)
  
  g_xc <- danio_cross_connections_graph
...
danio_cogs=gene_ids_cog
colnames(danio_cogs) <- c("id","cog")
sourceFolder("./ext_code/comparABle-main/comparABle-main/code/functions/", recursive = TRUE)
res <- cog_enrichment_analysis(x_modules = danio_id_module, x_cog = danio_cogs)

tm.a=t(res$enrichment)
tm.a[tm.a=="NaN"]=0
...
m_cog_cor[m_cog_cor+1 < 1] <- 0

g_m_cog_ppc <- graph_from_adjacency_matrix(
  adjmatrix = m_cog_cor,mode = "upper",diag = FALSE,weighted = TRUE
)
V(g_m_cog_ppc)$color <- translate_ids(V(g_m_cog_ppc)$name, dict = danio_modules_table[,c(4,7)])
V(g_m_cog_ppc)$border <- darken(translate_ids(V(g_m_cog_ppc)$name, dict = danio_modules_table[,c(4,7)]),0.5)
...
#GO
#{r}
colnames(danio_go) <- c("id","cog")
sourceFolder("./ext_code/comparABle-main/comparABle-main/code/functions/", recursive = TRUE)
res2 <- cog_enrichment_analysis(x_modules = danio_id_module, x_cog = danio_go)

m_go_cor <- cor(t(res2$enrichment))
#Heatmap(m_go_cor)
#hist(m_go_cor,breaks = 200, col = "#4c4c4c", border = NA)
m_go_cor[m_go_cor < 0.5] <- 0

g_m_go_ppc <- graph_from_adjacency_matrix(
  adjmatrix = m_go_cor,mode = "upper",diag = FALSE,weighted = TRUE
)
V(g_m_go_ppc)$color <- translate_ids(V(g_m_go_ppc)$name, dict = danio_modules_table[,c(4,7)])
V(g_m_go_ppc)$border <- darken(translate_ids(V(g_m_go_ppc)$name, dict = danio_modules_table[,c(4,7)]),0.5)
g_m_go_ppc = delete.vertices(g_m_go_ppc, which(igraph::degree(g_m_go_ppc)==0))
#
...
plot(
  ...
  g_m_go_ppc,
  edge.color = rgb(0,0,0,0.3),
  layout = layout.graphopt(g_m_go_ppc),
  vertex.frame.color = V(g_m_go_ppc)$border,
  vertex.label.dist=0.5,
  vertex.label.cex = 0.8,
  vertex.size = 6
)
...
# 5. Keep only those module connections supported by at least 1 "sources" (motif similarity, TF similarity, cross connections)
g_merge_aggr_2plus <- subgraph.edges(
  g_merge_aggr,
  eids = which(E(g_merge_aggr)$num_conections > 0),
  delete.vertices = TRUE
)
g_merge_aggr_2plus = delete.vertices(g_merge_aggr_2plus, which(igraph::degree(g_merge_aggr_2plus)==0))
V(g_merge_aggr_2plus)$cell_modulecolor <- translate_ids(V(g_merge_aggr_2plus)$name, danio_modules_table[,c(4,8)])
...
plot(
  ...
  g_m_cog_ppc,
  vertex.frame.color = V(g_m_cog_ppc)$border,
  vertex.color = V(g_m_cog_ppc)$color,#translate_ids(V(g_m_cog_ppc)$name, dict = danio_modules_table[,c(4,8)]),
  vertex.label.dist=0.5,
  vertex.label.cex = 0.8,
  layout = layout.graphopt(g_m_cog_ppc),
  vertex.size = 6
...
plot(
  g_merge_aggr,
  edge.color = rgb(0,0,0,0.3),
  vertex.color = V(g_merge_aggr)$color,#translate_ids(V(g_merge_aggr)$name, danio_modules_table[,c(4,8)]),
...
#{r}
pdf(paste0("./figures/",group,"/WGCNA/graph_merge.pdf"), width = 10, height = 10)
plot(
  g_merge_aggr_2plus, 
  edge.color = rgb(0,0,0,0.3),
  vertex.color = V(g_merge_aggr_2plus)$cell_modulecolor,
...
pdf(paste0("./figures/",group,"/WGCNA/graph_merge_modulecol.pdf"), width = 10, height = 10)
plot(
  g_merge_aggr_2plus,
  edge.color = rgb(0,0,0,0.3),
...
pdf(paste0("./figures/",group,"/WGCNA/graphs_merge_supp.pdf"), width = 24, height = 24)
par(mfrow = c(2,3))
set.seed(1)
plot(
  main = "tf correl",
...
pdf(paste0("./figures/",group,"/WGCNA/graphs_merge_complete_supp.pdf"), width = 10, height = 10)
set.seed(82)
plot(
  g_merge_aggr,
  edge.color = rgb(0,0,0,0.3),
...
save(
  ...
  new_cols,
  l,
  file = paste0("./data/",group,"/rda/wgcna_g_merge_aggr_2plus.rda")
)
}
...
wgcna_graph_merged=function(){
  ...
  string_db'
  string_prot=read.table(paste0("./data/string_db/7955.protein.info.v12.0.txt"),sep="\t")
  gene_ids <- string_prot$protein_external_id[
    na.omit(match(string_prot$preferred_name,danio_id_module$id))
  ]
  ...
  #{r aggregate_cross_connections_by_module}
  danio_cross_connections_norm_module <- aggregate(
    danio_cross_connections_norm[module_cols],
    by = list(Module = danio_cross_connections_norm$module),
    FUN = sum, drop=F
  )
